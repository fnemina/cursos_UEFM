...
\begin{frame}{\secname : \subsecname}
  ...
  \begin{itemize}
    \item \only<1>{\emph{Landsat 5}} \only<2>{Landsat 5 - \emph{TM}} \only<3-> {Landsat 5 - TM}
...
\begin{frame}{\secname : \subsecname}
\only<1>{Para clasificar a las plataformas nos vamos a centrar en la órbita que utilizan.}
\only<2->{
\begin{block}{Clasificación de órbitas}
   Una órbita es el camino que recorre un cuerpo en el espacio. Las dos más importantes para teledetección son:
   \begin{itemize}
     \item \emph{Polares}: Orbitando en torno a la tierra en dirección norte-sur o sur-norte.
     \item \emph{Geoestacionarias}: Orbitando a la tierra mirando siempre al mismo punto.
   \end{itemize}
\end{block}}
\end{frame}
%--- Next Frame ---%
