...
Esta clase tiene como objetivo familiarizarze con el entorno gráfico del \emph{SNAP (Sentinel Aplication Toolbox)}, un software de procesamiento de imágenes diseñado por la \emph{Agencia Espacial Europea (ESA)}, cuyas herramientas simplifican el trabajo con imágenes radar y ópticas.
...
A lo largo de la guía usaremos como referencia la siguiente simbología:
...
Para medir la distancia entre dos puntos o a lo largo de una poligonal se usa la herramienta \menu{Determines the distance between two points} (Figura \ref{fig:distancia}) que se encuentra en la barra de herramientas.
...
Para obtener información sobre un pixel seleccione la pestaña \menu{Pixel info}, junto al \menu{Product explorer} (Figura \ref{fig:pixel}), y posiciónese sobre uno en la imagen.
...
Para ver las tres en simultáneo haga click en \menu{Window>Tile Horizontally} mostrándose de esa manera las imágenes vinculadas una junto a la otra. Moverse o hacer zoom en una imagen, reproducirá esa acción en las otras. Para volver al modo habitual haga click en \menu{Window>Tile Single} (Figura \ref{fig:multiples}).

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.6\textwidth]{fig:multiples.png}
    \caption{Distintas imágenes, de la misma área geográfica, desplegadas en múltiples visualizadores en simultáneo.}
    \label{fig:multiples}
\end{figure}
